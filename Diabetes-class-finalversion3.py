...
upper_array_BMI = np.where(df['BMI'] >= upper_bmi)[0]
lower_array_BMI = np.where(df['BMI'] <= lower_bmi)[0]

df.drop(index=upper_array_BMI, inplace=True)
df.drop(index=lower_array_BMI, inplace=True)
...
df.drop('Diabetes_Class', axis=1).corrwith(df.Diabetes_Class).plot(kind='bar', grid=True, figsize=(10, 6), title="Correlation with Diabetes",color="deepskyblue")
plt.show();
...
Selected_df = df.drop(['Had_Chol_Check_In_Last_5_Years', 'Consumes_Fruits_Daily', 'Consumes_Vegetables_Daily', 'AnyHealthcare', 'NoDocbcCost', 'Education', "Income"], axis=1)
...
def knn_classifier(X_train, X_test, y_train, y_test):
    ...
    recall = st.write("KNN Recall", recall_score(y_test, y_test_pred_one, average='weighted')*100)
...
def rf_classifier(X_train, X_test, y_train, y_test):
    classifier_two = RandomForestClassifier(max_depth=20, n_estimators=30, random_state=1)
...
def mlp_classifier(X_train, X_test, y_train, y_test):
    classifier_three = MLPClassifier(activation='relu', solver='lbfgs', alpha=0.0001, max_iter=500,
...
with st.sidebar:
    ...
    bmi = st.sidebar.slider("BMI", 25, 30, 35)
    ...
    Diff_walk = st.sidebar.slider("Difficulty Walking", 0, 1)
    sex = st.sidebar.slider("Sex", 0, 1)
    age = st.sidebar.slider("Age", 0, 1)
    phys_health = st.sidebar.slider("Physical Health", 0, 1)
    hvy_alcohol_consump = st.sidebar.slider("Hvy Alcohol Consumption", 0, 1)
    men_hlth =st.sidebar.slider("Mental Health", 0, 1)
@st.cache_data
def rf_classifier_prediction(X_train, X_test, y_train, y_test):
    ...
    y_pred_tool_rf = st.sidebar.write("Random Forest Classification Prediction", rf.predict([[bmi, high_bp, high_chol, heart_disease_or_attack, phsical_activity, gen_hlth, smoker, stroke, men_hlth, hvy_alcohol_consump, sex, age, phys_health, Diff_walk]]))
...
def knn_classifier_prediction(X_train, X_test, y_train, y_test):
    ...
    y_pred_tool_knn = st.sidebar.write("KNN Classification Predication", knn.predict([[bmi, high_bp, high_chol, heart_disease_or_attack, phsical_activity, gen_hlth, smoker, stroke, men_hlth, hvy_alcohol_consump, sex, age, phys_health, Diff_walk]]))
...
def mlp_classifier_prediction(X_train, X_test, y_train, y_test):
    ...
    y_pred_tool_mlp = st.sidebar.write("MLP Classification Prediction", mlp.predict([[bmi, high_bp, high_chol, heart_disease_or_attack, phsical_activity, gen_hlth, smoker, stroke, men_hlth, hvy_alcohol_consump, sex, age, phys_health, Diff_walk]]))
