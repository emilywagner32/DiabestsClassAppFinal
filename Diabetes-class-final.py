...
st.info("This application aims to provide insight to the Diabetes Health Indicators Dataset through data exploration. Additionally this application showcases three classificationn models used in conjunction with the data set.")
...
tab1.write("Data is null value")
tab1.write(data.isnull().sum())

if tab1.button("Rename Columns", key="Rename Columns"):
     data.rename(columns={'Diabetes_012':'Diabetes_Class', 'CholCheck': 'Had_Chol_Check_In_Last_5_Years', 'PhysActivity': 'Phys_Activity_In_Last_30_Days', 'Fruits':'Consumes_Fruits_Daily', 'Veggies':'Consumes_Vegetables_Daily'},  inplace=True)


with tab1.expander("Show new Column names", expanded=False):
...
with tab1.expander("Show data type", expanded=False):
        st.write(df.dtypes)

with tab1.expander("Show new Dataframe", expanded=False):
        st.dataframe(df)
...
with tab1.expander('Data Description'):
...
with tab1.expander('Unbalanced Class Data'):
...
with tab1.expander('Balanced Class Data'):
...
with tab1.expander('Correlation Matrix'):
...
with tab1.expander('Density Barcharts'):
    st.caption("Density charts for highly correlated data points")
    visual3 = st.pyplot(fig3)

Selected_df.drop('Diabetes_Class', axis=1).corrwith(Selected_df.Diabetes_Class).plot(kind='bar', grid=True, figsize=(20, 8)
, title="Correlation with Diabetes_Class",color="Purple");
